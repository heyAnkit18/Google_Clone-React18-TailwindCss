...
const SearchResultHeader = () => {
  ...
  const { setImaageSearch } = useContext(Context);

  useEffect(() => {
    return () => setImaageSearch(false);
  }, []);

  const clickHandler = (menuItem) => {
    let isTypeImage = menuItem.name === "Images";
    setSelectedMenu(menuItem.name);
    setImaageSearch(isTypeImage ? true : false);
  };
    ...
    <div className="p-[15px] pb-0 md:pr-5 md:pl-20 md:pt-7 border-b border-[#ebebeb] flex md:block flex-col items-center sticky top-0 bg-white">
      ...
      <div className="flex ml-[-12px] mt-3">
        {menu.map((menu, index) => (
          <span
            key={index}
            className={`flex items-ccenter p-3 text-[#5f6368] cursor-pointer relative ${
